...
import { LITTLES_LAW_QUIZ } from "@/content/lesson-3-littles-law/quiz";
...
export function LittlesLawQuizStep({ onBack }: QuizStepProps) {
  ...
  const totalCorrect = LITTLES_LAW_QUIZ.filter((q) => answers[q.id] === q.ans).length;
  ...
  return (
    <div className="fade-up max-w-[720px]">
      <StepHeader
        ...
        desc="Five questions on Little's Law and what you observed in the simulation."
      />

      <div className="flex flex-col gap-4">
        {LITTLES_LAW_QUIZ.map((q) => {
      ...
      {totalAnswered === LITTLES_LAW_QUIZ.length && (
        <Card accent="59,130,246">
          <div className="text-center">
            <div className="text-2xl font-extrabold font-mono text-blue-400 mb-1">{totalCorrect}/{LITTLES_LAW_QUIZ.length}</div>
            <div className="text-xs" style={{ color: "var(--text-muted)" }}>
              {totalCorrect === LITTLES_LAW_QUIZ.length
                ? "Perfect score! You understand Little's Law."
                : totalCorrect >= 3
                ? "Good work! Review the explanations for any you missed."
                : "Try running the simulation again and watch how the variables interact."}
            </div>
          </div>
        </Card>
      )}
