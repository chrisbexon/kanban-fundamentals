...
export default function WipLimitsLesson() {
  ...
  useEffect(() => {
    ...
    loadGame().then((saved) => {
      if (saved) {
        game.loadSavedState(saved);
        if (saved.gameOver && saved.gameRound === 3) {
          setStep(4); // debrief
        } else if (saved.gameRound === 3) {
          setStep(3);
        } else if (saved.gameRound === 2) {
          setStep(2);
        } else {
          setStep(1);
        }
  ...
  // Auto-save when gameOver becomes true
  useEffect(() => {
    if (!loadedRef.current) return;
  ...
  const handleRoundFinish = () => {
    // Save current state at end of round
    saveGame({
      items: game.items,
      snapshots: game.snapshots,
      settings: game.settings,
